...
export class NgxCard {
  ...
  ngAfterViewInit() {
    new Card({
      ...
      width: this.width,
      formSelectors: {
        numberInput: this.findSelectors(this.numbers || undefined),
        expiryInput: this.findSelectors(this.expiries || undefined),
        cvcInput: this.findSelectors(this.cvcs || undefined),
        nameInput: this.findSelectors(this.names || undefined),
      },
  ...
  findSelectors(list?: QueryList<any>): string {
    return (list || [])
      .map(
        (template) =>
          template.elementRef.nativeElement.tagName.toLowerCase() +
          '[name="' +
          template.name +
          '"]'
      )
